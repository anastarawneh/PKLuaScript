    decrypted[0x88] = bit.band(decrypted[0x88], 0xFF00) + status
    encrypt_party_pokemon(slot, decrypted)
    return ";"..Command
end

function setHealth(slot, health)
    if slot > party_size or slot < 1 then
        return ";error;Slot is out of range."
    end
    local max = false
    ...
    elseif health < 0 then
        return ";error;HP value must be greater than 0."
    end
    local decrypted = decrypt_party_pokemon(slot)
    local max_health = decrypted[0x88 + 0x8]
    if max then
        health = max_health
    elseif health > max_health then
        return ";error;Provided HP value is greater than the max HP value."
    end
    decrypted[0x88 + 0x6] = health
    encrypt_party_pokemon(slot, decrypted)
    return ";"..Command
end
...
function edge(slot, amount)
    ...
    local species_id = decrypted[blockA + 0x0]
    local exp = decrypted[blockA + 0xA] * 0x10000 + decrypted[blockA + 0x8]
    local level = getLevel(exp, decrypted[blockA + 0x0]) + 1
    if level > 100 then
        return ";error;Cannot edge a level 100 Pokémon."
    end
    local experience = level_exp_curve[level][curves[species_id] + 1] - amount
    if experience < 0 or experience < level_exp_curve[level - 1][curves[species_id] + 1] then
        return ";error;Invalid experience amount."
    end
    decrypted[blockA + 0x8] = bit.band(experience, 0xFFFF)
    decrypted[blockA + 0xA] = bit.rshift(experience, 16)
    encrypt_party_pokemon(slot, decrypted)
    return ";"..Command
end

function resetPP(slot)
    local decrypted, pid = decrypt_party_pokemon(slot)
    local shuffle_key = bit.rshift(bit.band(pid, 0x3E000), 0xD) % 24 + 1
    local blockA = shuffle[shuffle_key][1]
    local blockB = shuffle[shuffle_key][2]
    local blockC = shuffle[shuffle_key][3]
    local blockD = shuffle[shuffle_key][4]
    local species = decrypted[blockA + 0x0]
    local move1 = decrypted[blockB + 0x0]
    local move2 = decrypted[blockB + 0x2]
    local move3 = decrypted[blockB + 0x4]
    local move4 = decrypted[blockB + 0x6]
    local pp1 = move_pp[move1] or 0
    local pp2 = move_pp[move2] or 0
    local pp3 = move_pp[move3] or 0
    local pp4 = move_pp[move4] or 0
    decrypted[blockB + 0x8] = pp2 * 0x100 + pp1
    decrypted[blockB + 0xA] = pp4 * 0x100 + pp3
    encrypt_party_pokemon(slot, decrypted)
    return ";"..Command
end
...
function encrypt_data(offset, decrypted, seed)
    data = {}
    for i = 0, table.maxn(decrypted), 2 do
        seed = mult32(seed, 0x41C64E6D) + 0x6073
        encrypted_word = bit.bxor(decrypted[i], bit.rshift(seed, 16))
        memory.writeword(offset + i, encrypted_word)
    end
end
...
function try_decrypt_pokemon(mon_offset, box)
    ...
    local checksum2 = 0
    for i = 0x8, 0x86, 2 do 
        checksum2 = checksum2 + decrypted[i]
        checksum2 = bit.band(checksum2, 0xFFFF)
    end
...
function encrypt_party_pokemon(slot, decrypted)
    mon_offset = party_offset + party_mon_size * (slot - 1)
    local pid = memory.readdword(mon_offset)
    local checksum = 0
    for i = 0x8, 0x86, 2 do
    ...
    local decrypted_tables = {}
    for i = 0x8, 0x86, 2 do
        decrypted_tables[i - 0x8] = decrypted[i]
    end
    encrypt_data(mon_offset + 0x8, decrypted_tables, checksum)
    local decrypted_battle = {}
    for i = 0x88, 0xEA, 2 do
        decrypted_battle[i - 0x88] = decrypted[i]
    end
    encrypt_data(mon_offset + 0x88, decrypted_battle, pid)
    memory.writeword(mon_offset + 0x6, checksum)
end
